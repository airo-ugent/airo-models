...
- **Visual meshes must be `.obj`** — Drake does not support STL for visual geometry. When sourcing meshes from upstream (which often provides `.STL`), convert them with `trimesh` before adding:
  ```bash
  python3 -c "import trimesh; trimesh.load('mesh.STL').export('mesh.obj')"
  ```
- Collision meshes are already `.obj` (generated by the convex decomposition scripts).
